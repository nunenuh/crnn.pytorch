...
class Attention(nn.Module):
    ...
    def _char_to_one_hot(self, input_char: torch.Tensor, one_hot_dim: int = 38):
        ...
        one_hot = torch.FloatTensor(batch_size, one_hot_dim).zero_().to(device)
        return one_hot
    
    def forward(self, batch_H, text, is_train: bool = True, batch_max_length: int = 25):
        """
        Args:
            batch_H ([type]): contextual_feature H = hidden state of encoder. [batch_size x num_steps x contextual_feature_channels]
